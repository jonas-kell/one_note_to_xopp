...
use onenote_parser::contents::{
    EmbeddedFile, Image, Ink, Outline, OutlineElement, OutlineItem, RichText, Table,
};
...
fn main() {
    ...
    for element in std::path::Path::new(r"./").read_dir().unwrap() {
        ...
        if let Some(extension) = in_file_path.extension() {
            if extension == "one" {
                let file_name_without_extension = in_file_path
                    .file_stem()
                    .unwrap_or_default()
                    .to_string_lossy();
                ...
                for page_series in section.page_series() {
                    for page in page_series.pages() {
                        ...
                        let file_name = sanitize_filename::sanitize(format!(
                            "{} - {}",
                            file_name_without_extension, page_name
                        ));
...
fn output_file(page_xml: String, file_name: String) {
    ...
    if CFG.output_xml {
        fs::write(format!("{}.xml", file_name), file_output.clone())
            .expect("Couldn't write xml file");
    }
    ...
    gz.write_all(&(file_output.into_bytes()))
        .expect("Couldn't write .xopp file");
...
fn render_page(page: &Page) -> String {
    ...
    page_output.push_str(&format!(
        "<page width=\"{}\" height=\"{}\">
        <background type=\"solid\" color=\"#ffffffff\" style=\"graph\"/>
        <layer>\n",
        size_watcher.size_x + CFG.page_padding,
        size_watcher.size_y + CFG.page_padding
    ));
...
fn render_outline_content(
    content: &Content,
    outline_offset: Option<(f32, f32)>,
    size_watcher: &mut SizeWatcher,
) -> String {
    match content {
...
fn render_image(
    ...
    let offset_horizontal = image.offset_horizontal().unwrap_or_else(|| 0.0)
        * CFG.image_offset_factor()
        + outline_offset.unwrap_or((0.0, 0.0)).0 * CFG.outline_offset_factor();
    let offset_vertical = image.offset_vertical().unwrap_or_else(|| 0.0)
        * CFG.image_offset_factor()
        + outline_offset.unwrap_or((0.0, 0.0)).1 * CFG.outline_offset_factor();
    ...
    let mut image_content = String::from(format!(
...
fn render_ink(ink: &Ink, size_watcher: &mut SizeWatcher) -> String {
    ...
    let offset_horizontal = ink.offset_horizontal().unwrap_or_default();
    let offset_vertical = ink.offset_vertical().unwrap_or_default();
    ...
    for ink_stroke in ink.ink_strokes() {
        if ink_stroke.path().len() >= 2 {
            ...
            image_content.push_str(&format!(
                "{} {} ",
                size_watcher.check_x(last_x * CFG.ink_scaling_factor()),
                size_watcher.check_y(last_y * CFG.ink_scaling_factor())
            ));
            for point in ink_stroke.path()[1..].iter() {
                image_content.push_str(&format!(
                    "{} {} ",
                    size_watcher.check_x((last_x + point.x()) * CFG.ink_scaling_factor()),
                    size_watcher.check_y((last_y + point.y()) * CFG.ink_scaling_factor())
                ));
                last_x = last_x + point.x();
                last_y = last_y + point.y();
            }
            if ink_stroke.path().len() < 4 {
                // if not enough points, will fail
                image_content.push_str(&format!(
                    "{} {} ",
                    size_watcher.check_x(last_x * CFG.ink_scaling_factor()),
                    size_watcher.check_y(last_y * CFG.ink_scaling_factor())
